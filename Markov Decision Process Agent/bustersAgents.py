...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for dist in livingGhostPositionDistributions:
            most_probably = dist.argMax()
            most_likely_pos.append(most_probably)

        act=None
        current=99999999999999999
        for action in legal:
            pacman_next_pos = Actions.getSuccessor(pacmanPosition,action)
            for pos_ghost in most_likely_pos:
                dist_ghost=self.distancer.getDistance(pacman_next_pos,pos_ghost)
                if dist_ghost < current:
                    current=dist_ghost
                    act = action
        return act
